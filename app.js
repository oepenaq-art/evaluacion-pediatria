...
async function generateFinalReport() {
    ...
    // 4. Generar análisis con Cloud Function (IA) o Síntesis Local Fallback
    showLoading('Generando análisis cualitativo...');
    let aiAnalysis = '';
    try {
        const generarInforme = functions.httpsCallable('generarInformeFinal');
        const res = await generarInforme({
            resName, rotation, avgFinal, itemAverages,
            fortalezas: allFortalezas, mejoras: allMejoras,
            dateFrom, dateTo, microcurriculo: MICROCURRICULOS[rotation]
        });
        if (res.data && res.data.text) {
            aiAnalysis = res.data.text;
        } else {
            aiAnalysis = generateDescriptiveAnalysis(resName, rotation, avgFinal, itemAverages, allFortalezas, allMejoras);
        }
    } catch (e) {
        console.warn('Cloud Function no disponible o en desarrollo, usando síntesis cualitativa local:', e);
        aiAnalysis = generateDescriptiveAnalysis(resName, rotation, avgFinal, itemAverages, allFortalezas, allMejoras);
    }
...
/* ========================================================
   SÍNTESIS CUALITATIVA DE LA ROTACIÓN (Local)
   ======================================================== */
